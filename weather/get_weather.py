...
from WeatherMeizuJson import WeatherDecode
from send_msg_email import SendEmail
...
class Weather:
    ...
    http://www.weather.com.cn/weather1d/101281601.shtml
    default city: dongguan
    author: AsherYang
    create: 2016.11.29
    """

    def __init__(self, city='东莞'):
        self.city = city
    ...
    def getWeather(self):
        url = "http://www.weather.com.cn/weather1d/" + str(getCity(self.city)) + ".shtml"
        html = urllib2.urlopen(url).read()
        tree = lxml.html.fromstring(html)
        print tree.cssselect('div#today > input#hidden_title')
        hidden_weather = tree.cssselect('div#today > input#hidden_title')[0]
        # seven_day = tree.cssselect('div#7d > input#hidden_title')
        # print seven_day
        return hidden_weather.get('value')

    # http://aider.meizu.com/app/weather/listWeather?cityIds=101281601
    def getWeatherByMeizu(self):
        url = "http://aider.meizu.com/app/weather/listWeather?cityIds=" + str(getCity(self.city))
        html = urllib2.urlopen(url).read()
        print html
        weatherValue = json.loads(html, encoding='utf8', cls=WeatherDecode)
        # currentCity = weatherValue.city
        # weatherDetailsInfo = weatherValue.weatherDetailsInfo
        # weathers = weatherValue.weathers
        # weather3HoursDetailsInfos = weatherDetailsInfo.weather3HoursDetailsInfos
        # print 'currentCity = %s , weatherDetailsInfo = %s, weathers = %s' % (currentCity, weatherDetailsInfo, weathers)
        return weatherValue
...
if __name__ == '__main__':
    ...
    # weather.getWeather()
    # weather_now = city + ' ' + weather.getWeather()
    meizuWeather = weather.getWeatherByMeizu()
    weather_now = meizuWeather.city + meizuWeather.weathers.date.replace('-', '') + meizuWeather.weathers.week + \
                  meizuWeather.weathers.weather + meizuWeather.weathers.temp_day_c + '/' + \
                  meizuWeather.weathers.temp_night_c + u'℃'
    #print "weather_now = ", weather_now
    sendEmail = SendEmail()
    sendEmail(content=weather_now)
